...
const RequestPage = ({navigation}) => {
    ...
    const handleNextButtonPress = () => {
        setOpen(false);
        const purchaseFee : number = purchaseCost;
        ...
        //RequestDetail 화면에서 GET 요청으로 데이터를 얻는 방식으로 변경해야 함.
        ...
        axios.post(apiUrl, JSON.stringify(requestData),{
          ...
          .then(response => {
              ...
              navigation.navigate('Home'); // 다음 화면으로 이동
          })
          .catch(error => {
              // 요청이 실패한 경우에 수행할 동작
    ...
    const data_categories = [
        {key: '일거리 요청', value: '일거리 요청'},
        {key: '분리수거 배출', value: '분리수거 배출'},
        {key: '가정집 청소', value: '가정집 청소'},
    ];
    const data_timing = [
        {key: '즉시수행', value: '즉시수행'},
        {key: '예약수행', value: '예약수행'},
